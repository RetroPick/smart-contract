...
import {MarketEngineStorage} from "./storage/MarketEngineStorage.sol";
...
contract MarketEngine is Initializable, ReentrancyGuardTransient, UUPSUpgradeable, MarketEngineStorage {
    using SafeERC20 for IERC20;
    using MarketTypes for MarketTypes.Epoch;
    using MarketMath for MarketTypes.Ledger;

    /// @dev Selector routing for facets (delegatecall targets).
    mapping(bytes4 selector => address facet) internal facetOf;
    ...
    modifier onlyWorkerOrAdmin() {
        if (msg.sender != _admin && msg.sender != _workerAuthority) revert Unauthorized();
        _;
    }

    modifier onlyTreasuryOrAdmin() {
        if (msg.sender != _treasury && msg.sender != _admin) revert Unauthorized();
        _;
    }

    modifier notPausedUserOps() {
        if (_globalPaused) revert ProtocolPaused();
        _;
    }
    ...
    function initialize(
        ...
        if (_configInitialized) revert Unauthorized();
        ...
        _admin = admin_;
        _treasury = treasury_;
        _workerAuthority = worker_;
        _defaultSettlementFeeBps = defaultSettlementFeeBps_;
        _maxSwitchFeeBps = maxSwitchFeeBps_;
        _maxOutcomes = maxOutcomes_;
        _oracleConfig = MarketTypes.OracleConfig({
    ...
    /// @notice Set facet routing for function selectors.
    /// @dev Can be invoked atomically as the `data` parameter during a UUPS upgrade.
    function setFacets(bytes4[] calldata selectors, address[] calldata facets) external onlyAdmin {
        uint256 n = selectors.length;
        if (n != facets.length) revert InvalidTemplate();
        for (uint256 i = 0; i < n; i++) {
            facetOf[selectors[i]] = facets[i];
        }
    }
    ...
    function setYieldRouter(address router, uint16 feeBps) external onlyAdmin {
        ...
        address old = address(_yieldRouter);
        _yieldRouter = IYieldRouter(router);
        _yieldFeeBps = router == address(0) ? 0 : feeBps;
        emit YieldRouterSet(old, router, _yieldFeeBps);
    }
